...
// Facet number variable refers to a special variable "$fn" that is used to control the number of facets to generate an
// arc. Higher than 100 is not recommended according to the docs as it demands a lot resources
...
vertical_keepout_size_factor =
    1.1; // Expand calculated keepout region by this factor. 1=no expansion, 1.5=50% expansion, etc
...
function get_letter_for_front(flap_index) = flap_index >= len(character_list)
                                                ? get_letter_for_front(flap_index - len(character_list))
...
module _draw_letter(letter, flap_gap)
{
    ...
    thickness_offset = is_undef(overrides[5])
                           ? (is_undef(get_font_setting("thickness_offset")) ? 0 : get_font_setting("thickness_offset"))
                           : overrides[5];
    ...
    if (len(color_index) > 0 && is_num(color_index[0]))
    {
        ...
        {
            square([ flap_width - flap_notch_depth * 2 - 4, (flap_height * 2 + flap_gap) * color_height_with_default ],
                   center = true);
        }
    }
    else
        ...
        { // valign compensation
            scale([ width, 1, 1 ])
            {
                translate([ offset_x, offset_y ])
                {
                    ...
                    {
                        text(text = letter, size = flap_height * height * 2, font = get_font_setting("font"),
                             halign = "center", $fn = letter_facet_number);
                    }
                }
            }
        }
...
module _flap_letter(letter, letter_color, flap_gap, front = true, bleed = 0, print_3d = false)
{
    ...
    {
        translate([
            0, 0, front ? (flap_thickness / 2 + eps - (print_3d ? font_extrusion_3d : 0)) : (-flap_thickness / 2 - eps)
        ])
        {
            linear_extrude(height = print_3d ? font_extrusion_3d : font_extrusion, center = print_3d ? false : true)
            {
                intersection()
                {
                    difference()
                    {
                        ...
                        // Note that the verticle keepout (if applicable) is subtracted *after* the bleed is applied;
                        // otherwise text would bleed into the keepout zone.
                        if (vertical_keepout_mode > 0)
                        {
                            vertical_keepout_width =
                                flap_width + bleed * 2; // keepout must expand by "bleed" on the left and right
                        ...
                        // TODO: consider adding horizontal keepout; it may be undesirable for text to meet the sides of
                        // the flaps (or possibly extend beyond the sides if bleed > 0).
                    }
...
module flap_with_letters(flap_color, letter_color, flap_index, flap_gap, flap = true, front_letter = true,
                         back_letter = true, bleed = 0, print_3d = false)
{
